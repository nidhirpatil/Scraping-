...
try:
	for i in range(50):
		r= requests.get("http://www.moneycontrol.com/stocks/company_info/stock_news.php?sc_id=GAC&scat=&pageno={}&next=0&durationType=Y&Year=2018&duration=1&news_type=".format(i), allow_redirects = False)
		...
		for link in links:
			...
			try:
				...
				extract_text = data_text.find(class_ = 'arttidate')
				print extract_text.get_text()
				
				headlines = data_text.find(class_ = 'article_box') 
				text_data = headlines.get_text()
				print text_data
				print '\n'
